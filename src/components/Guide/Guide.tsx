...
export const Guide = ({ serverApi, fullscreen }: GuideProps) => {
    ...
    const handleDismiss = (updatedGuide: GuideContents) => {
        dispatch({ type: ActionType.UPDATE_GUIDE, payload: updatedGuide });
    };
...
const FullScreenGuide = ({ serverApi, onDismiss }: FullGuideProps) => {
    const { state } = useContext(AppContext);
    useEffect(() => {
        return function cleanup() {
            state.currentGuide && onDismiss(state.currentGuide);
        };
    }, [state.currentGuide]);
    return (
        <div
